(function () {
    const sheet = document.getElementById("education_section");
    ...
    function computeMinTranslatePx() {
      ...
      return Math.max(0, sheetH - getViewportHeight() + topGapPx);
    }
  
    function computePeekTranslatePx() {
      const vh = getViewportHeight() / 100;
    ...
    function computeHiddenTranslatePx() {
      const sheetH = sheet.getBoundingClientRect().height;
      return Math.max(peekTranslatePx, sheet.getBoundingClientRect().height - 24) - 60;
    }
    ...
    if (window.visualViewport) {
      ...
      // Chrome mobile sometimes changes the usable viewport when the bottom bar expands/collapses
